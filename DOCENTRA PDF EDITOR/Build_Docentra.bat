...
REM Taşınabilir SDK yolunu belirle (Üst dizindeki dotnet_sdk klasörü)
set "DOTNET_SDK=%~dp0..\dotnet_sdk\dotnet.exe"
...
REM Dosyayı Standalone klasörüne kopyala (Kullanıcının beklediği yer)
set "SOURCE_EXE=bin\Release\net8.0-windows10.0.17763.0\win-x64\publish\Docentra.exe"
set "DEST_DIR=bin\Release\Standalone"

if exist "%SOURCE_EXE%" (
    echo "Copying to Standalone folder..."
    copy /y "%SOURCE_EXE%" "%DEST_DIR%\Docentra.exe"
)

echo.
echo "Build Complete! Updated exe is in: %DEST_DIR%"
pause

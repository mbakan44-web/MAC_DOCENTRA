    ...
    public class PdfService
    {
        ...
        public async Task<bool> CompressPdfAsync(string sourcePath, string targetPath, string level = "Low")
        {
            return await Task.Run(() =>
            {
                try
                {
                    using (PdfDocument doc = PdfReader.Open(sourcePath, PdfDocumentOpenMode.Import))
                    {
                        using (PdfDocument output = new PdfDocument())
                        {
                            ...
                            if (level == "Medium" || level == "High")
                            {
                                int quality = level == "Medium" ? 75 : 40;
                                OptimizeImages(output, quality);
                            }
                ...
                {
                    LogMessage($"Compress Error: {ex.Message}");
                    return false;
                }
            });
        }

        private void OptimizeImages(PdfDocument doc, int quality)
        {
            try
            {
                ...
                {
                    if (obj is PdfDictionary dict && dict.Elements.GetString("/Subtype") == "/Image")
                    {
                        try
                        {
                            // Sadece JPEG veya ham görüntüleri işlemeyi dene
                            var filter = dict.Elements.GetString("/Filter");
                            if (filter == "/DCTDecode" || string.IsNullOrEmpty(filter))
                            {
                                byte[]? imageBytes = dict.Stream?.Value;
                                ...
                                using (var ms = new System.IO.MemoryStream(imageBytes))
                                {
                                    using (var originalImage = System.Drawing.Image.FromStream(ms))
                                    {
                                        using (var compressedMs = new System.IO.MemoryStream())
                                        {
                                            ...
                                            originalImage.Save(compressedMs, jpegEncoder, encoderParameters);
                                            dict.Stream.Value = compressedMs.ToArray();
                        ...
                        catch { /* Bazı görüntüler işlenemeyebilir, atla */ }
                    }
                }
            }
            catch { }
        ...
        public async Task<string?> SplitPagesAsync(string sourcePath, string rangeInput = null)
        {
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    // Masaüstü kirliliğini önlemek için alt klasör oluştur
                    string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                    string subFolderName = $"{name}_Parcalanmis_{timestamp}";
                    string targetDir = Path.Combine(dir, subFolderName);
                    ...
                    using (PdfDocument sourceDoc = PdfReader.Open(sourcePath, PdfDocumentOpenMode.Import))
                    {
                        if (string.IsNullOrWhiteSpace(rangeInput))
                        {
                            ...
                            {
                                string outPath = Path.Combine(targetDir, $"{name}_Sayfa_{i + 1}.pdf");
                                using (PdfDocument targetDoc = new PdfDocument())
                                {
                        ...
                        else
                        {
                            ...
                            if (targetPages.Count == 0) return null;

                            string outPath = Path.Combine(targetDir, $"{name}_Split.pdf");
                            using (PdfDocument targetDoc = new PdfDocument())
                            {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    using (PdfDocument doc = PdfReader.Open(sourcePath, PdfDocumentOpenMode.Modify))
                    {
                        ...
                        // Producer is read-only in PdfSharp
                ...
                {
                    LogMessage($"SetMetadata Error: {ex.Message}");
                    return false;
                }
            });
        }

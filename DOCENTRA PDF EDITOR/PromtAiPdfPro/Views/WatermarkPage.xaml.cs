        ...
        public WatermarkPage()
        {
            ...
            Loaded += async (s, e) =>
            {
                if (!string.IsNullOrEmpty(Helpers.NavigationHelper.PendingFilePath))
                {
                    ...
                    if (action == "TextWatermark")
                    {
                        _textSourcePdf = path;
                        TxtTextSelectedPdf.Text = System.IO.Path.GetFileName(path);
                        ...
                        var thumbnail = await _pdfService.GetPdfThumbnailAsync(path);
                        if (thumbnail != null)
                        {
                    ...
                    else if (action == "LogoWatermark")
                    {
                        _logoSourcePdf = path;
                        TxtLogoSelectedPdf.Text = System.IO.Path.GetFileName(path);

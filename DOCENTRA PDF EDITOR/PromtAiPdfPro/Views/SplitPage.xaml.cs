...
namespace PromtAiPdfPro.Views
{
    public partial class SplitPage : Page
    {
        ...
        private async void BtnSplit_Click(object sender, RoutedEventArgs e)
        {
            ...
            string? resultDir = await _pdfService.SplitPagesAsync(TxtSourceFile.Text, range);
            ...
            if (Application.Current.MainWindow is MainView mainWindow)
            {
                if (resultDir != null)
                {
                    ...
                    {
                        System.Diagnostics.Process.Start(new System.Diagnostics.ProcessStartInfo(resultDir) { UseShellExecute = true });
                    }
                }
                else
                {
